...
def make_train_val_filelist(path, split_ratio=0.9, seed=42, shuffle=True):

    wav_files = glob(os.path.join(path, "**", "*.wav"), recursive=True)
    ...
    train_filelist = wav_files[:n_train]
    validation_filelist = wav_files[n_train:]

    return train_filelist, validation_filelist
